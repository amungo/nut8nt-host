...
#define XILINX_DMA_COALESCE_MAX		(1024 * 16) //defius
#define XILINX_DMA_NUM_DESCS		(1024 * 16)
	...
	unsigned long flags)
{
	...
	if (!num_periods) {
		printk(KERN_ERR "num_periods\n");
		return NULL;
	}

	if (!is_slave_direction(direction)) {
		printk(KERN_ERR "direction\n");
		return NULL;
	}
...
static int xilinx_dma_probe(struct platform_device *pdev)
{
	...
	printk("xdev->dma_config->dmatype == %d\n", xdev->dma_config->dmatype);
