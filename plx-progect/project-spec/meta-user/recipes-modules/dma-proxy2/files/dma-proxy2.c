...
static void cyclic_callback(void *data) {
    ...
    if ((pchannel_p->loopcount % pchannel_p->sig_per) == pchannel_p->sig_per - 1) {
        send_signal(pchannel_p, (pchannel_p->loopcount) % pchannel_p->buffer_table_p->buf_num);
    }
...
static void transfer_cyclic(struct dma_proxy_channel *pchannel_p)
{
	...
	//pchannel_p->bdtable[bdindex].dma_handle = (dma_addr_t)(pchannel_p->interface_phys_addr +
	...
	printk(" transfer_cyclic %d %d\n", interface_p->length, interface_p->buf_num);
	chan_desc = dma_device->device_prep_dma_cyclic(pchannel_p->channel_p, pchannel_p->bdtable[bdindex].dma_handle, interface_p->length * interface_p->buf_num, interface_p->length, pchannel_p->direction, flags);
	//chan_desc = dma_device->device_prep_slave_sg(pchannel_p->channel_p, pchannel_p->bdtable[bdindex].sglist, 1, 
	//					pchannel_p->direction, flags, NULL);
