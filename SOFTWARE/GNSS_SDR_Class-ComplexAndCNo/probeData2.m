...
if (nargin == 1)
    ...
    fileNameStr1 = settings.fileName1;
    fileNameStr2 = settings.fileName2;
elseif (nargin == 2)
...
[fid, message] = fopen(fileNameStr1, 'rb');
...
if (fid > 0)
   ...
   [data, count] = fread(fid, [1, dataAdaptCoeff*100*samplesPerCode], settings.dataType);
    ...
    error('Unable to read file %s: %s.', fileNameStr1, message);
end % if (fid > 0)



[fid, message] = fopen(fileNameStr2, 'rb');
...
if (fid > 0)
   ...
   [data, count] = fread(fid, [1, dataAdaptCoeff*100*samplesPerCode], settings.dataType);
    ...
    %=== Error while opening the data file ================================
    error('Unable to read file %s: %s.', fileNameStr2, message);
end % if (fid > 0)
